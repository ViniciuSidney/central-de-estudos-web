...
export function initNotes() {
  ...
  const noteFilterSubjectSelect = document.querySelector(
    "#note-filter-subject",
  );
  const noteFilterThemeSelect = document.querySelector("#note-filter-theme");
  const noteFilterTypeSelect = document.querySelector("#note-filter-type");
  const noteFilterStatusSelect = document.querySelector("#note-filter-status");
  ...
  const toggleNoteFiltersButton = document.querySelector(
    "#toggle-note-filters",
  );
  ...
  function renderNotes() {
    ...
    notes.forEach((note) => {
      ...
      const visibleMetadata = getVisibleNoteMetadata(
        note,
        subjectName,
        themeName,
      );
  ...
  function getFilteredNotes() {
    ...
    const selectedTag = noteFilterTagSelect.value;
    const selectedFlag = noteFilterFlagSelect.value;
    ...
    return getNotes()
      .filter((note) => {
        ...
        const matchesTheme =
          !selectedThemeId || note.themeId === selectedThemeId;

        const matchesType = !selectedType || note.type === selectedType;

        const matchesStatus =
          !selectedStatus || (note.status || "rascunho") === selectedStatus;
  ...
  function toggleAdvancedNoteFilters() {
    ...
    toggleNoteFiltersButton.textContent = filtersAreHidden
      ? "Ocultar filtros"
      : "Mostrar filtros";

    toggleNoteFiltersButton.setAttribute(
      "aria-expanded",
      String(filtersAreHidden),
    );
  }
  ...
  document.addEventListener("click", (event) => {
    const clickedInsideNoteMenu = event.target.closest(".note-card__more");

    if (clickedInsideNoteMenu) {
      return;
    }
  ...
  noteFilterTagSelect.addEventListener("change", renderNotes);
  noteFilterFlagSelect.addEventListener("change", renderNotes);

  clearNoteFiltersButton.addEventListener("click", clearNoteFilters);
