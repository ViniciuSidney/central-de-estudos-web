...
export function initSubjects() {
	...
	function getSubjectThemesCount(subjectId) {
		return getThemes().filter((theme) => {
			return theme.subjectId === subjectId;
		}).length;
	}

	function formatSubjectThemesCount(total) {
		return total === 1 ? '1 tema cadastrado' : `${total} temas cadastrados`;
	}
	...
	function renderSubjects() {
		...
		subjects.forEach((subject) => {
			...
			const themesCount = getSubjectThemesCount(subject.id);
        ...
        <div class="subject-card__content">
          ...
          <span class="subject-card__themes-count ${themesCount === 0 ? 'is-empty' : ''}">
            ${formatSubjectThemesCount(themesCount)}
          </span>
  ...
  `;
	...
	document.addEventListener('themes:changed', renderSubjects);
