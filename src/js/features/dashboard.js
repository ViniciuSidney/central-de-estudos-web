...
export function initDashboard() {
	...
	const subjectPerformanceList = document.querySelector('#dashboard-subject-performance');
	...
	const topErrorSubjectsList = document.querySelector('#dashboard-top-error-subjects');
	const topErrorThemesList = document.querySelector('#dashboard-top-error-themes');
	const topPendingSubjectsList = document.querySelector('#dashboard-top-pending-subjects');

	const topPendingThemesList = document.querySelector('#dashboard-top-pending-themes');
	...
	function formatCount(value, singularLabel, pluralLabel) {
		...
		return normalizedValue === 1 ? `1 ${singularLabel}` : `${normalizedValue} ${pluralLabel}`;
	}
	...
	function getAttemptDate(attempt) {
		return attempt.createdAt || attempt.answeredAt || attempt.resolvedAt || attempt.date || attempt.updatedAt || null;
	}

	function getReviewDate(review) {
		return review.reviewedAt || review.createdAt || review.updatedAt || review.answeredAt || review.date || null;
	}
	...
	function getPendingErrorAttempts() {
		...
		wrongAttempts.forEach((attempt) => {
			...
			if (!savedAttempt || new Date(getAttemptDate(attempt)) > new Date(getAttemptDate(savedAttempt))) {
		...
		return Array.from(lastWrongAttemptByQuestion.values())
			...
			.sort((firstAttempt, secondAttempt) => {
				return new Date(getAttemptDate(secondAttempt)) - new Date(getAttemptDate(firstAttempt));
			});
	}
	...
	function getGeneralStats() {
		...
		const accuracy = attempts.length > 0 ? (correctAttempts.length / attempts.length) * 100 : 0;
	...
	function renderReviewCards(stats) {
		reviewCards.innerHTML = [
			...
			createCard({
				...
				value: formatPercent(stats.wrongAttempts.length > 0 ? (stats.reviewedErrors.length / stats.wrongAttempts.length) * 100 : 0),
	...
	function renderRecentActivity(stats) {
		const recentAttempts = stats.attempts.map((attempt) => {
			...
			return {
				...
				description: question ? `${getSubjectName(question.subjectId)} • ${getThemeName(question.themeId)}` : 'Resolução registrada.',
				status: getAttemptResult(attempt) === 'correct' ? 'Acertou' : getAttemptResult(attempt) === 'wrong' ? 'Errou' : 'Registrada',
		...
		const recentReviews = stats.errorReviews.map((review) => {
			...
			return {
				...
				description: isReviewPending(review) ? 'Erro pendente de revisão.' : 'Erro transformado em regra de correção.',
		...
		if (activities.length === 0) {
			renderEmptyState(recentActivityList, 'Nenhuma atividade registrada ainda.', 'Resolva questões ou revise erros para movimentar o Dashboard.');
			return;
		}
	...
	function getSubjectPerformance() {
		...
		return subjects
			.map((subject) => {
				...
				const accuracy = attemptsFromSubject.length > 0 ? (correctAttempts.length / attemptsFromSubject.length) * 100 : 0;
	...
	function renderSubjectPerformance() {
		...
		if (subjectPerformance.length === 0) {
			renderEmptyState(subjectPerformanceList, 'Nenhum desempenho por matéria ainda.', 'Resolva questões para visualizar indicadores por matéria.');
			return;
		}
	...
	function getPriorityNotes() {
		return getNotes()
			.filter((note) => {
				...
				return note.status === 'revisar' || note.status === 'importante' || note.isPinned || note.isFavorite;
			})
	...
	function renderPriorityNotes() {
		...
		if (priorityNotes.length === 0) {
			renderEmptyState(priorityNotesList, 'Nenhuma anotação prioritária.', 'Anotações fixadas, favoritas, importantes ou para revisar aparecerão aqui.');
			return;
		}
	...
	function renderContentAlerts() {
		...
		if (alerts.length === 0) {
			renderEmptyState(contentAlertsList, 'Nenhum ponto de atenção encontrado.', 'Sua base de conteúdos está bem preenchida até aqui.');
			return;
		}
	...
	function showDashboardTab(tabName) {
		...
		document.querySelector('#dashboard-summary-tab')?.classList.toggle('is-active', tabName === 'summary');

		document.querySelector('#dashboard-performance-tab')?.classList.toggle('is-active', tabName === 'performance');

		document.querySelector('#dashboard-reviews-tab')?.classList.toggle('is-active', tabName === 'reviews');

		document.querySelector('#dashboard-contents-tab')?.classList.toggle('is-active', tabName === 'contents');
	}
	...
	function getThemePerformance() {
		...
		return themes
			.map((theme) => {
				...
				const accuracy = attemptsFromTheme.length > 0 ? (correctAttempts.length / attemptsFromTheme.length) * 100 : 0;
	...
	function renderTopErrorSubjects() {
		...
		if (topErrorSubjects.length === 0) {
			renderEmptyState(topErrorSubjectsList, 'Nenhuma matéria com erro registrada.', 'Quando houver respostas incorretas, as matérias com mais erros aparecerão aqui.');
			return;
		}
        ...
        <article class="dashboard-rank-item">
          ...
          <div>
            <strong>${escapeHTML(item.subject.name)}</strong>
            <span>
              ${formatCount(item.wrong, 'erro', 'erros')} • ${formatCount(item.correct, 'acerto', 'acertos')} • ${formatCount(item.attempts, 'tentativa', 'tentativas')}
            </span>
          </div>
	...
	function renderTopErrorThemes() {
		...
		if (topErrorThemes.length === 0) {
			renderEmptyState(topErrorThemesList, 'Nenhum tema com erro registrado.', 'Quando houver respostas incorretas, os temas com mais erros aparecerão aqui.');
			return;
		}
	...
	function getTopPendingSubjects() {
		...
		return Array.from(pendingBySubject.values()).sort(sortRankingByCount).slice(0, 5);
	}
	...
	function getTopPendingThemes() {
		...
		pendingErrors.forEach((review) => {
			...
			const subjectName = question.subjectId ? getSubjectName(question.subjectId) : 'Matéria removida';
		...
		return Array.from(pendingByTheme.values()).sort(sortRankingByCount).slice(0, 5);
	}
	...
	function renderTopPendingSubjects() {
		...
		if (topPendingSubjects.length === 0) {
			renderEmptyState(topPendingSubjectsList, 'Nenhuma matéria com pendência.', 'Quando houver erros pendentes, as matérias mais recorrentes aparecerão aqui.');
			return;
		}
        ...
        <article class="dashboard-rank-item">
          ...
          <div>
            ...
            <span>
              ${formatCount(item.count, 'erro pendente', 'erros pendentes')}
            </span>
          </div>
	...
	function renderTopPendingThemes() {
		...
		if (topPendingThemes.length === 0) {
			renderEmptyState(topPendingThemesList, 'Nenhum tema com pendência.', 'Quando houver erros pendentes, os temas mais recorrentes aparecerão aqui.');
			return;
		}
	...
	function getContentAlertMessage(alert) {
		...
		const entityLabel = escapeHTML(alert.entityLabel);
		const middleText = escapeHTML(alert.middleText);
		const missingText = escapeHTML(missingLabel[alert.missing] || 'conteúdos');
		const suffix = escapeHTML(alert.suffix);

		return `<strong class="dashboard-entity-highlight">${entityLabel}</strong>${middleText}<strong class="dashboard-missing-word">${missingText}</strong>${suffix}`;
	}
	...
	function handleDashboardReviewErrorOpen(event) {
		const openButton = event.target.closest('[data-open-dashboard-review-error]');
