...
export function initSolve() {
  ...
  const solveNoQuestionWarning = document.querySelector(
    "#solve-no-question-warning",
  );
  ...
  const solveQuestionContext = document.querySelector(
    "#solve-question-context",
  );
  const solveQuestionStatus = document.querySelector("#solve-question-status");
  const solveQuestionStatement = document.querySelector(
    "#solve-question-statement",
  );
  ...
  function renderAlternativeButtons(question) {
    ...
    renderableAlternatives.forEach((alternative, index) => {
      ...
      alternativeButton.dataset.originalAlternative =
        alternative.originalLetter;
  ...
  function revealCorrection(question) {
    document.querySelectorAll(".solve-alternative").forEach((button) => {
      ...
      if (
        originalAlternative === selectedOriginalAlternative &&
        selectedOriginalAlternative !== question.correctAlternative
      ) {
  ...
  function confirmAnswer() {
    ...
    solveFeedback.hidden = false;
    solveFeedback.className = isCorrect
      ? "solve-feedback is-correct"
      : "solve-feedback is-wrong";

    const selectedAlternativeText =
